    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> RecoverPassword(RecoverPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                string myToken = await _userHelper.GeneratePasswordResetTokenAsync(user); //genera un token
                string link = Url.Action(
                    "ResetPassword",
                    "Account", //el cual va al controller y pasa al mètodo ResetPassword
                    new { token = myToken }, protocol: HttpContext.Request.Scheme); //por debajo envìa el mismo token creado
